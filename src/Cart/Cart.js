...
export default class HomeScreen extends React.Component {
  ...
  decrement(id) {
    for (var i = 0; i < global.cart.length; i++) {
      if (global.cart[i].name == id) {
        global.cart[i].num = global.cart[i].num - 1;
        if(global.cart[i].num == 0){
          this.removeItem(id);
        }
      }
    }
  ...
  generateItems = () => {
    ...
    global.cart.forEach(item => {
      ...
      total += combined;
      items.push(<CartItem item={item.name} quantity={item.num} price={item.price} onPress={() => this.removeItem(item.name)} onAdd={() => this.increment(item.name)} onRemove={() => this.decrement(item.name)} />)
    });
